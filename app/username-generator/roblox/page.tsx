...
export function generateMetadata({ params }: { params: { platform: string } }): Metadata {
  const meta = PLATFORM_META['roblox']
  return {
    title: meta.title,
...
const jsonLd = serializeSchemas(
  buildWebApplication({ name: 'Roblox Username Generator', url: 'https://hplaptop.co/username-generator/roblox', description: 'Generate unique Roblox Username Generator ideas — free, instant, no sign-up required.' }),
  buildBreadcrumb([{ name: 'Username Generator', path: '/username-generator' }, { name: 'Roblox Username Generator', path: '/username-generator/roblox' }]),
  buildFAQ(PLATFORM_CONTENT['roblox'].faq.map((f) => ({ q: f.q, a: f.a })))
)

export default function PlatformGeneratorPage() {
  const content = PLATFORM_CONTENT['roblox']
  ...
  return (
    <>
      ...
      <div className="card mb-12">
        <UsernameGenerator defaultPlatform="roblox" />
      </div>
