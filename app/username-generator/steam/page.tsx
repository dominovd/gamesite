...
const jsonLd = serializeSchemas(
  buildWebApplication({ name: 'Steam Username Generator', url: 'https://hplaptop.co/username-generator/steam', description: 'Generate unique Steam Username Generator ideas — free, instant, no sign-up required.' }),
  buildBreadcrumb([{ name: 'Username Generator', path: '/username-generator' }, { name: 'Steam Username Generator', path: '/username-generator/steam' }]),
  buildFAQ(PLATFORM_CONTENT['steam'].faq.map((f) => ({ q: f.q, a: f.a })))
)
