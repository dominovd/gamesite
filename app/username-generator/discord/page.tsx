...
export function generateMetadata({ params }: { params: { platform: string } }): Metadata {
  ...
  return {
    ...
    alternates: { canonical: 'https://hplaptop.co/username-generator/discord' },
  }
}
...
const jsonLd = serializeSchemas(
  buildWebApplication({ name: 'Discord Username Generator', url: 'https://hplaptop.co/username-generator/discord', description: 'Generate unique Discord Username Generator ideas — free, instant, no sign-up required.' }),
  buildBreadcrumb([{ name: 'Username Generator', path: '/username-generator' }, { name: 'Discord Username Generator', path: '/username-generator/discord' }]),
  buildFAQ(PLATFORM_CONTENT['discord'].faq.map((f) => ({ q: f.q, a: f.a })))
)

export default function PlatformGeneratorPage() {
  const content = PLATFORM_CONTENT['discord']
  ...
  return (
    <>
      ...
      <div className="card mb-12">
        <UsernameGenerator defaultPlatform="discord" />
      </div>
