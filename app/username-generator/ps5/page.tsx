...
export function generateMetadata({ params }: { params: { platform: string } }): Metadata {
  const meta = PLATFORM_META['ps5']
  return {
    title: meta.title,
    description: meta.description,
    keywords: meta.keywords,
    alternates: { canonical: 'https://hplaptop.co/username-generator/ps5' },
  }
}
...
const jsonLd = serializeSchemas(
  buildWebApplication({ name: 'PS5 Username Generator', url: 'https://hplaptop.co/username-generator/ps5', description: 'Generate unique PS5 Username Generator ideas — free, instant, no sign-up required.' }),
  buildBreadcrumb([{ name: 'Username Generator', path: '/username-generator' }, { name: 'PS5 Username Generator', path: '/username-generator/ps5' }]),
  buildFAQ(PLATFORM_CONTENT['ps5'].faq.map((f) => ({ q: f.q, a: f.a })))
)

export default function PlatformGeneratorPage() {
  const content = PLATFORM_CONTENT['ps5']
  ...
  return (
    <>
      ...
      <div className="card mb-12">
        <UsernameGenerator defaultPlatform="ps5" />
      </div>
